  ...
  <md-card md-with-hover class="card-style round-display">
    ...
    <div v-for="(map, index) in rounds" :key="map.id" :id="map.id">
      ...
      <md-card-content class="row-contents">
        ...
        <div class="column-contents">
          <md-checkbox v-model="map.show" @change="showMapClick($event, map.id)"
            >Display Map</md-checkbox
          >
        </div>
        <md-checkbox
          v-model="map.state"
...
export default {
  ...
  methods: {
    ...
    showMapClick(event, mapInfo) {
      this.$emit("roundMap", { show: event, mapId: mapInfo });
    },
